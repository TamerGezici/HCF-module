function [varargout] = fMRIDataMasking_mahalanobis_dummy_beta(fullBrainVols, binaryMasks_nS, betaCorrespondence, userOptions);
% If beta doesn't exist, creates dummy beta after noise normalization
	...
	for subject = 1:nSubjects % and for each subject...
        ...
        for run_no=1:subj_total_runs
            for regressor=1:size(betas_of_interest,1)
                ...
                elseif isempty(beta_index)
                    % IF BETA DOESN'T EXIST, then create a dummy.
                    maskVoxelPatterns(:,regressor,run_no) = ones(1,size(normalisedPatterns,2));
                end
            end
        end
