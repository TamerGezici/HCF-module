    ...
    for sub = 1:size(subj_list,2)
        ...
        if isfile([event_dir,'.csv'])
            ...
            for session = 1:n_sessions % runs (or tasks)
                ...
                participant_sessions_list{session} = session_name;
                ...
                present_regressors{session} = ismember(evnames, unique_session_regressors);
                for regressor = 1:(size(unique_session_regressors,2)) % loop through regressor types
            ...
            for session = 1:n_sessions % runs (or tasks)
                ...
                participant_sessions = strcat(participant_sessions,session_name,'+'); % Sessions which will be processed will be stored in this variable.
                participant_sessions_list{session} = session_name;
                event_numbers = [1:1:n_conditions];
                unique_session_regressors = sort(unique(session_onsets(:,1)))';
                present_regressors{session} = ismember(event_numbers, unique_session_regressors);
        ...
        if ~isempty(contrasts)
            for session=1:size(participant_sessions_list,2)
                %participant_sessions = participant_sessions(1:end-1); % removes the "+" at the end
                session_name = participant_sessions_list{session};
                aa_session_specifier = ['singlesession:',session_name];
                for contrast=1:size(contrast_list,2) % loop over your contrasts
                    curr_contrast = contrast_list{contrast};
                    curr_contrast_vector = contrasts(curr_contrast);
                    session_vector = present_regressors{session};
                    missing_indices = [find(session_vector==0)];
                    curr_contrast_vector(missing_indices) = [];
                    aap = aas_addcontrast(aap, 'aamod_firstlevel_contrasts', subject_number, aa_session_specifier,curr_contrast_vector,curr_contrast, 'T');
                end
            end
        end
    end
