...
function [aa_structure] = generate_model(aap,subj_list,events_folder,evnames,contrast_list,contrasts)
    missing_event_files_acknowledged = false;
    ...
    for sub = 1:size(subj_list,2)
        ...
        if isempty(file_ext) && ~missing_event_files_acknowledged
            ...
            switch choice
                case 'Ignore all subjects'
                    missing_event_files_acknowledged = true;
                case 'Stop'
        ...
        if strcmp(file_ext,'.mat') % otherwise, just load them as matrices.
            ...
            if n_conditions == 0
                error('No events found! Check your events folder and/or event names.');
            end
            events = [];
