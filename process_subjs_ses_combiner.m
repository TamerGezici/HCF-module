...
function [aa_structure] = process_subjs(subj_list,aap,DATA_PATH,session_identifier,tasknames,process_fmaps,process_4D,alternative)
    ...
    sessions_list = {};
    session_list_length = 1;
    if isa(session_identifier, 'containers.Map')
        session_container = session_identifier;
    end

    if isa(session_identifier, 'cell')
        sessions_list = session_identifier;
        session_list_length = size(sessions_list,2);
    end

    for bids_session = 1:session_list_length

        for sub=1:size(subj_list,2)
            subj = subj_list{sub}; % Get the subject names
            ...
            if isa(session_identifier, 'cell')
                session_identifier = sessions_list{bids_session};
            end
