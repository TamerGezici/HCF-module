function hcf_cvManovaSearchlight(dirName, slRadius, Cs, permute, lambda, subject_name, output_dir_path)
...
%% This section is added by Adem, This section is the modified part of the original commented section above
%%% this modication enabled to incorporate the subject names in the resulting folders
cd(output_dir_path)

for ci = 1 : nContrasts
    for pi = 1 : nPerms
        spmWriteImage(reshape(D(:, ci, pi), size(mask)), ...
            sprintf('%s_spmD_C%04d_P%04d.nii', subject_name, ci, pi), ...
            misc.mat, 'descrip', 'pattern discriminability');
        spmWriteImage(reshape(D(:, ci, pi) ./ sqrt(p), size(mask)), ...
            sprintf('%s_spmDs_C%04d_P%04d.nii', subject_name, ci, pi), ...
            misc.mat, 'descrip', 'standardized pattern discriminability');
    end
end

% Save voxels per searchlight as an image for each subject
spmWriteImage(reshape(p, size(mask)), sprintf('%s_VPSL.nii', subject_name), ...
              misc.mat, 'descrip', 'voxels per searchlight');

% Save analysis parameters for each subject
save(sprintf('%s_cmsParameters.mat', subject_name), 'slRadius', 'Cs', 'permute', 'misc', 'nPerms');
